        ...
        if f"emu{cat}" in dir_name:  # Filtrar según la categoría que nos interesa TODO: Debug por si la categoría no existe
            ...
            with open(output_txt_path, "w") as f:
                ...
                for bin in range(1, n_bins + 1):
                    ...
                    for hist_key in hist_keys: # hist_VV, hist_ttbar, hist_Higgs, ...
                        hist = hist_key.ReadObj()
                        if not hist or not isinstance(hist, r.TH1) or not hist.InheritsFrom("TH1"): # or hist.GetBinLowEdge(bin) < mejor_corte_var1: <-- TODO: 2D ES ESTO???????
                            continue
...
if __name__ == "__main__":
    ...
    write_header1 = not os.path.exists(os.path.join(output_txt_dir, "FoM_optimos_var1.txt")) or os.stat(os.path.join(output_txt_dir, "FoM_optimos_var1.txt")).st_size == 0
    write_header2 = not os.path.exists(os.path.join(output_txt_dir, "FoM_optimos_var2.txt")) or os.stat(os.path.join(output_txt_dir, "FoM_optimos_var2.txt")).st_size == 0

    with open(os.path.join(output_txt_dir, "FoM_optimos_var1.txt"), "a", newline = "") as f:
        if write_header1:
            f.write("Punto de masa    Corte óptimo    Señal    Fondo    FoM \n")
        f.write(f"({mchi}, {mphi})    {corte1}    {señal_optima1}    {fondo_optimo1}    {fom1} \n")

    with open(os.path.join(output_txt_dir, "FoM_optimos_var2.txt"), "a", newline = "") as f:
        if write_header2:
            f.write("Punto de masa    Corte óptimo    Señal    Fondo    FoM \n")
